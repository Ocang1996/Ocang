...
const Sidebar = ({ onLogout, activeItem = 'dashboard', onExpandChange }: SidebarProps) => {
  const location = useLocation();
  const [mobileOpen, setMobileOpen] = useState(false);
  // Simpan state expanded dalam localStorage agar konsisten di semua halaman
  ...
  // Handle sidebar expansion with callback to parent
  ...
  // Update parent component setiap kali status expanded berubah
  useEffect(() => {
    if (onExpandChange) {
  ...
  return (
    <>
      ...
      <aside
        className={`hidden lg:flex flex-col fixed left-6 top-1/2 -translate-y-1/2 z-30 ${expanded ? 'w-56' : 'w-16'} transition-all duration-300 ease-in-out`}
      >
        <div className="py-6 px-3 bg-white/60 dark:bg-gray-800/60 backdrop-blur-lg border border-gray-200/30 dark:border-gray-700/30 rounded-2xl shadow-lg">
          <div className="flex flex-col items-center space-y-2">
            ...
            {navItems.map((item) => (
              ...
              >
                {!expanded ? (
                  // Compact view - click only expands the sidebar
                  <div
                  ...
                  >
                    ...
                    <span className="ml-3 text-sm font-medium whitespace-nowrap overflow-hidden transition-all">{item.name}</span>
                  </Link>
                )}
              </div>
            ))}
